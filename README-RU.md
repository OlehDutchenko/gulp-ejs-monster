...
Вы можете передать собственные свойства и методы, которые будут достпны Вам внутри шаблоназотора в объекте `locals`
...
Выводит историю ренедера после завершения работы с каждой страницей.
...
Контент текущей страницы, для вставки внутри лейаутов. Соотвественно свойтство доступно только внутри лейатов! 
...
Ввходящие параметры для текущего виджета. Соотвественно свойтство доступно только внутри виджетов! 
...
```markup
<!-- widget test.ejs -->
<% let title = localse.entry.title || 'Default title'; %>
<h1><%- title) %></h1>
```

```markup
<!-- widget test.ejs using es6 syntax -->
...
Флаг, изменился ли файл после последнего обращения к нему.   Свойство достпно внутри виджетов и инклудов. На основных страницах ренедера и из лейаутах, свойство также доступно, но для них оно всегда будет равно `true`.
